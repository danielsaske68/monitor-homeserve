...
def lista_servicios(servicios):
    return {
        "inline_keyboard": [
            [{"text": s["servicio"], "callback_data": f"SEL_{s['servicio']}"}]
            for s in servicios
        ] + [[{"text": "⬅️ Volver", "callback_data": "BACK_MENU"}]]
    }
...
class HomeServe:
    ...
    def obtener(self):
        try:
            ...
            bloques = re.split(r"\n(?=\d{7,8}\s)", text)
            return {
                re.search(r"\b\d{7,8}\b", b).group(0): " ".join(b.split())
                for b in bloques if re.search(r"\b\d{7,8}\b", b)
            }
        except:
            return {}
    ...
    def obtener_servicios_curso(self):
        try:
            ...
            for fila in filas[1:]:
                ...
                if len(cols) >= 6:
                    m = re.search(r"\d{7,8}", cols[0].get_text())
                    if m:
                        servicios.append({
                            "servicio": m.group(0),
    ...
    def detalle_servicio(self, sid):
        try:
            ...
            data = {"servicio": sid, "direccion": "", "poblacion": "", "comentarios": ""}
...
def webhook():
    ...
    if "callback_query" in data:
        ...
        if action == "CURSO":
            ...
            tg_edit(chat, msg_id, "📋 Selecciona servicio:", lista_servicios(servicios))
