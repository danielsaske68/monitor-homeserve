...
BASE_URL = os.getenv("BASE_URL", "https://monitor-homeserve.onrender.com")  # URL pública
...
def bot_loop():
    ...
    if not homeserve.login():
        logger.error("No se pudo iniciar sesión en HomeServe")
        return
    while True:
        try:
...
def telegram_webhook():
    ...
    if "message" in data:
        ...
        if text == "/start":
            enviar(chat_id, "Bienvenido al monitor de HomeServe")
