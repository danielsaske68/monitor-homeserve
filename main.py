...
def enviar(chat, texto, botones=None, editar=False):
    ...
    try:
        if editar and chat in PANEL:
            ...
            http.post(f"{TELEGRAM_API}/editMessageText", json=data, timeout=10)
        else:
            r = http.post(f"{TELEGRAM_API}/sendMessage", json=data, timeout=10)
            if r.ok:
...
def webhook():
    ...
    if "callback_query" in data:
        ...
        if accion == "LOGIN":
            enviar(chat, "✅ Login OK" if homeserve.login() else "❌ Error", botones(), True)

        elif accion == "REFRESH":
            enviar(chat, f"🔄 {len(homeserve.obtener())} servicios", botones(), True)
            ...
            enviar(chat, "Selecciona servicio", lista_servicios(homeserve.obtener_curso()), True)
