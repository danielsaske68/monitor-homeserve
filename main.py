...
# ---------------- DATABASE ----------------
# Ruta de DB persistente en Railway
DB_PATH = "/var/lib/containers/railwayapp/bind-mounts/695080cd-74cc-4fef-b8e2-8bbf3479ff28/vol_otk7v841omd3up84/usuarios.db"
...
class HomeServe:
    ...
    def cambiar_estado(self, servicio_id, estado):
        try:
            ...
            return (True, f"✅ Estado {estado} aplicado correctamente") if r.status_code == 200 else (False, f"⚠️ Error HTTP {r.status_code}")
    ...
    def aceptar_servicio(self, servicio_id):
        try:
            ...
            r = self.session.post(BASE_URL, data=payload, timeout=10)
            return (True, f"✅ Servicio {servicio_id} aceptado") if r.status_code == 200 else (False, f"⚠️ Error aceptando")
        except Exception as e:
            return False, f"❌ Error: {e}"
    ...
    def rechazar_servicio(self, servicio_id):
        try:
            ...
            r = self.session.post(BASE_URL, data=payload, timeout=10)
            return (True, f"❌ Servicio {servicio_id} rechazado") if r.status_code == 200 else (False, f"⚠️ Error rechazando")
        except Exception as e:
            return False, f"❌ Error: {e}"
