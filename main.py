...
def enviar_estado_servicio(chat, servicio_id, texto):
    botones = {
        "inline_keyboard": [
            [
                {"text": "Pendiente", "callback_data": f"ESTADO_PENDIENTE_{servicio_id}"},
                {"text": "En Progreso", "callback_data": f"ESTADO_PROGRESO_{servicio_id}"},
                {"text": "Finalizado", "callback_data": f"ESTADO_FINALIZADO_{servicio_id}"}
            ],
            [
                {"text": "🔄 Refresh", "callback_data": "CAMBIAR_ESTADO"}
            ]
        ]
    }
    requests.post(
...
class HomeServe:
    ...
    def obtener(self):
        ...
        try:
            ...
            logger.error(f"Error obtener asignación: {e}")
            return {}

    def obtener_servicios_en_curso(self):
        """Servicios en curso para cambiar estado"""
        try:
...
def telegram_webhook():
    ...
    if "callback_query" in data:
        ...
        # ---------------- CAMBIAR ESTADO SERVICIOS EN CURSO ----------------
        elif accion == "CAMBIAR_ESTADO":
            servicios = homeserve.obtener_servicios_en_curso()
            if not servicios:
                enviar(chat, "⚠️ No hay servicios en curso")
            else:
                for idserv, servicio in servicios.items():
                    enviar_estado_servicio(chat, idserv, f"🛠 <b>Servicio en curso</b>\n\n{servicio}")

        elif accion.startswith("ESTADO_"):
            partes = accion.split("_")
            estado = partes[1]
            servicio_id = partes[2]
            SERVICIOS_ESTADO[servicio_id] = estado
            enviar(chat, f"✅ Servicio {servicio_id} cambiado a {estado}")
...
if __name__ == "__main__":
    ...
    app.run(host="0.0.0.0", port=port)        "inline_keyboard": [
            [
                {"text": "🔐 Login", "callback_data": "LOGIN"},
                {"text": "🔄 Refresh", "callback_data": "REFRESH"}
            ],
